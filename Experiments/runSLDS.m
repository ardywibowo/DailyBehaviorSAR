...
bestLogLikelihood = -realmax;
for r = 1 : numEMRestarts
	
	[bmiImputedSARTemp, UisImputedSARTemp, ASarTemp, BjsSarTemp, CSarTemp, ...
		sigma2SARTemp, sTransSARTemp, pStGivenX1TsSARTemp, logLikelihoodsSAR, initDTemp, initPStGivenX1TsTemp] = ...
		inputGroupSARLearn(bmiTrain, inputTrain, orderX, orderU, numStates, tSkip, numEMIterations, imputeParamsSAR);
	
	if logLikelihoodsSAR(length(logLikelihoodsSAR)) > bestLogLikelihood(length(bestLogLikelihood))
		initD = initDTemp;
		initPStGivenX1Ts = initPStGivenX1TsTemp;
		bmiImputedSAR = bmiImputedSARTemp;
		UisImputedSAR = UisImputedSARTemp;
		ASar = ASarTemp;
		BjsSar = BjsSarTemp;
		CSar = CSarTemp;
		sigma2SAR = sigma2SARTemp;
		sTransSAR = sTransSARTemp;
		pStGivenX1TsSAR = pStGivenX1TsSARTemp;
		bestLogLikelihood = logLikelihoodsSAR;
	end
end
...
save([resultsDirectory, 'initD.mat'], 'initD');
save([resultsDirectory, 'initPStGivenX1Ts.mat'], 'initPStGivenX1Ts');
...
for i = 1 : numSubjects
	bmiTest{i} = [bmiTrain{i} bmiMissing{i}];
end
